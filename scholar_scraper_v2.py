...
def process_profile(url):
    try:
        ...
        auhor_name = profile_data['name']

        process_citation_data(user_id, auhor_name)
...
def process_citation_data(user_id, author_name):
    citation_data = get_citation_data(user_id)

    df_citation_data = pd.DataFrame(citation_data['cited_by'])
    df_citation_data.to_csv(author_name+"'s Profile.csv", index=False)
